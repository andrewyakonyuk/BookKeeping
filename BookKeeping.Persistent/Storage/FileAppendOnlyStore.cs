...
namespace BookKeeping.Persistent.Storage
{
    public class FileAppendOnlyStore : IAppendOnlyStore
    {
        ...
        // caches
        private readonly ConcurrentDictionary<string, DataWithVersion[]> _items = new ConcurrentDictionary<string, DataWithVersion[]>();

        private readonly ReaderWriterLockSlim _thread = new ReaderWriterLockSlim();

        private DataWithName[] _all = new DataWithName[0];
        ...
        private static bool TryReadRecord(BinaryReader binary, out Record result)
        {
            result = null;
            try
            {
        ...
        private void AddToCaches(string key, byte[] buffer, long commit)
        {
        ...
        private IEnumerable<Record> EnumerateHistory()
        {
        ...
        private void PersistInFile(string key, byte[] buffer, long commit)
        {
